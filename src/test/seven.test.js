...
const allWinAllPlay = trump => {
  check([7, 2, 1], [7 * ppt(trump), 2 * ppt(trump), 1 * ppt(trump)], trump);
  check([7, 3, 0], [7 * ppt(trump), 3 * ppt(trump), 0 * ppt(trump)], trump);
};
...
const playersWinByOneAllPlay = trump => {
  check(
    [6, 2, 2],
    [-1 * (10 * ppt(trump) + 4 * ppt(trump)), 2 * ppt(trump), 2 * ppt(trump)],
    trump,
  );
  check(
    [6, 3, 1],
    [-1 * (10 * ppt(trump) + 4 * ppt(trump)), 3 * ppt(trump), 1 * ppt(trump)],
    trump,
  );
  check(
    [6, 4, 0],
    [-1 * (10 * ppt(trump) + 4 * ppt(trump)), 4 * ppt(trump), 0 * ppt(trump)],
    trump,
  );
};
...
const playersWinByOneHelp = trump => {
  check(
    [6, 2, 2],
    [-1 * (10 * ppt(trump) + 4 * ppt(trump)), 0 * ppt(trump), 4 * ppt(trump)],
    trump,
    ROLE.helper,
  );
  check(
...
const bidderWinByOneAllPlay = trump => {
  check([8, 1, 1], [8 * ppt(trump), -9 * ppt(trump), 1 * ppt(trump)], trump);
  check([8, 2, 0], [8 * ppt(trump), 2 * ppt(trump), -10 * ppt(trump)], trump);
  check([8, 0, 2], [8 * ppt(trump), -10 * ppt(trump), 2 * ppt(trump)], trump);
};

const bidderWinByOneHelp = trump => {
  check(
    [8, 1, 1],
    [8 * ppt(trump), 0 * ppt(trump), -8 * ppt(trump)],
    trump,
    ROLE.helper,
  );
  check(
...
const bidderWinByManyAllPlay = trump => {
  check([9, 1, 0], [18 * ppt(trump), -9 * ppt(trump), -9 * ppt(trump)], trump);
  check(
    [9, 0, 1],
    [9 * pptSolo(trump), -10 * pptSolo(trump), 1 * pptSolo(trump)],
    trump,
  );
  check(
    [10, 0, 0],
    [
      10 * pptSolo(trump) + 10 * ppt(trump),
      -10 * pptSolo(trump),
      -10 * ppt(trump),
    ],
    trump,
...
const bidderWinByManyHelp = trump => {
  ...
  check(
    [9, 0, 1],
    [9 * pptSolo(trump), 0 * pptSolo(trump), -9 * pptSolo(trump)],
    trump,
    ROLE.helper,
  );
  check(
